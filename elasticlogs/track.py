...
class ElasticlogsBulkSource:
    def __init__(self, indices, params):
        ...
        if 'index' not in params.keys():
            if len(indices) > 1:
                logger.info("More than one index specified in track configuration. Will use the first one ({})".format(indices[0]['name']))
            else:
                logger.info("Using index specified in track configuration ({})".format(indices[0]['name']))

            self._params['index'] = indices[0]['name']
            self._default_index = True

        else:
            logger.info("Index pattern specified in parameters ({}) will be used".format(params['index']))
